...
def load_model():
    ...
    try:
        ...
        # Завантаження референсних даних для drift детекції
        ...
        if os.path.exists(reference_data_path):
            ...
            # Генерація мок референсних даних
...
class MockModel:
    """Мок-модель для тестування"""
    def predict(self, X):
        """Просте передбачення на основі середнього значення features"""
        X = np.array(X)
        if X.ndim == 1:
            X = X.reshape(1, -1)
...
class SimpleDriftDetector:
    """Простий drift детектор на основі статистики"""
    ...
    def predict(self, X, return_instance_score=True):
        ...
        # Проста перевірка: якщо середнє значення сильно відрізняється від 0
        mean_val = np.mean(np.abs(X))
        drift_score = mean_val
        
        # Дрейф якщо середнє значення більше порогу
